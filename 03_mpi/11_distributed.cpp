...
int main(int argc, char** argv) {
  ...
  Body ibody[N/size], jbody[N/size], buff_body[N/size];
  srand48(rank);
  ...
  MPI_Win win;
  MPI_Win_create(buff_body, N/size*sizeof(Body), sizeof(Body), MPI_INFO_NULL, MPI_COMM_WORLD, &win);
  for(int irank=0; irank<size; irank++) {
    MPI_Win_fence(0, win);
    MPI_Put(jbody, N/size, MPI_BODY, send_to, 0, N/size, MPI_BODY, win);
    MPI_Win_fence(0, win);
    memcpy(jbody, buff_body, N/size*sizeof(Body));
